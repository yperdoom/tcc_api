...
const setUserToTokenize = require('../factory/setUserToTokenize')
...
module.exports.getAllClients = async (requisition, response, next) => {
  ...
  const query = { 'client.manager_id': params.managerUserId }

  if (params.find) {
    query.name = { $regex: params.find }
  }

  const clients = await User.get(query)
...
module.exports.getAllManagers = async (requisition, response, next) => {
  const params = requisition.query
  let query = { scope: 'manager' }

  if (params.find) {
    query.name = { $regex: params.find }
  }

  const managers = await User.get(query)
